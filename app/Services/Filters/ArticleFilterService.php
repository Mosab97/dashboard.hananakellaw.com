...
class ArticleFilterService
{
    ...
    {
        Log::info('Starting to apply article filters with params:', $params);

        foreach ($params as $key => $value) {
            if ($value !== null && $value !== '') {
                Log::info("Applying article filter for: $key with value:", ['value' => $value]);
                $this->applyFilter($query, $key, $value);
            } else {
                Log::info("Skipping article filter for: $key as the value is null or empty.");
            }
        }

        logQuery($query);
        Log::info('All article filters applied.');
    ...
    protected function applyFilter($query, $key, $value)
    {
        switch ($key) {
            ...
            case 'date_range':
                ...
                break;
            default:
                Log::warning("Unknown article filter key: $key");
                break;
        }
    }
