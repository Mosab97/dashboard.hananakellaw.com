...
class SucessStoryFilterService
{
    /**
    ...
    {
        Log::info('Starting to apply sucess story filters with params:', $params);

        foreach ($params as $key => $value) {
            if ($value !== null && $value !== '') {
                Log::info("Applying sucess story filter for: $key with value:", ['value' => $value]);
                $this->applyFilter($query, $key, $value);
            } else {
                Log::info("Skipping sucess story filter for: $key as the value is null or empty.");
            }
        }

        logQuery($query);
        Log::info('All sucess story filters applied.');
    ...
    protected function applyFilter($query, $key, $value)
    {
        switch ($key) {
            ...
            case 'date_range':
                ...
                break;
            default:
                    Log::warning("Unknown sucess story filter key: $key");
                break;
        }
    }
    ...
    private function filterBySearch($query, $value)
    {
        $query->where(function ($query) use ($value) {
            ...
            foreach ($locales as $locale) {
                $query->orWhereRaw(
                    "LOWER(JSON_UNQUOTE(JSON_EXTRACT(owner_name, '$.{$locale}'))) LIKE ?",
                    ['%'.strtolower($value).'%']
                );
            }
        });
