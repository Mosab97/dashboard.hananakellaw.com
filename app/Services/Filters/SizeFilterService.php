...
class SizeFilterService
{
    ...
    {
        Log::info('Starting to apply size filters with params:', $params);

        foreach ($params as $key => $value) {
            if ($value !== null && $value !== '') {
                Log::info("Applying size filter for: $key with value:", ['value' => $value]);
                $this->applyFilter($query, $key, $value);
            } else {
                Log::info("Skipping size filter for: $key as the value is null or empty.");
            }
        }

        logQuery($query);
        Log::info('All size filters applied.');
    ...
    protected function applyFilter($query, $key, $value)
    {
        switch ($key) {
            ...
            case 'date_range':
                ...
                break;
            default:
                Log::warning("Unknown size filter key: $key");
                break;
        }
    }
