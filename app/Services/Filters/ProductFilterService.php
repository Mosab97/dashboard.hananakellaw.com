...
class ProductFilterService
{
    ...
    {
        Log::info('Starting to apply product filters with params:', $params);

        foreach ($params as $key => $value) {
            if ($value !== null && $value !== '') {
                Log::info("Applying product filter for: $key with value:", ['value' => $value]);
                $this->applyFilter($query, $key, $value);
            } else {
                Log::info("Skipping product filter for: $key as the value is null or empty.");
            }
        }

        logQuery($query);
        Log::info('All product filters applied.');
    ...
    protected function applyFilter($query, $key, $value)
    {
        switch ($key) {
            ...
            case 'restaurant_id':
                ...
                break;
            case 'category_id':
                $this->filterByCategory($query, $value);
                break;
            case 'active':
            ...
            case 'date_range':
                ...
                break;
            default:
                Log::warning("Unknown product filter key: $key");
                break;
        }
    }
     ...
     * Filter by category
     */
    private function filterByCategory($query, $value)
    {
        if (is_array($value)) {
