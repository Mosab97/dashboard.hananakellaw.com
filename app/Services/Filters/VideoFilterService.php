...
class VideoFilterService
{
    ...
    {
        Log::info('Starting to apply video filters with params:', $params);

        foreach ($params as $key => $value) {
            if ($value !== null && $value !== '') {
                Log::info("Applying video filter for: $key with value:", ['value' => $value]);
                $this->applyFilter($query, $key, $value);
            } else {
                Log::info("Skipping video filter for: $key as the value is null or empty.");
            }
        }

        logQuery($query);
        Log::info('All video filters applied.');
    ...
    protected function applyFilter($query, $key, $value)
    {
        switch ($key) {
            ...
            case 'date_range':
                ...
                break;
            default:
                Log::warning("Unknown video filter key: $key");
                break;
        }
    }
