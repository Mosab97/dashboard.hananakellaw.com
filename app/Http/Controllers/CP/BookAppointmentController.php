...
use App\Services\Filters\BookAppointmentFilterService;
...
class BookAppointmentController extends Controller
{
    ...
    public function __construct(
        BookAppointment $_model,
        BookAppointmentFilterService $filterService,
    ) {
        $this->config = config('modules.book-appointment');
        $this->_model = $_model;
        ...
        if ($request->isMethod('POST')) {
            ...
            return DataTables::eloquent($items)
                ...
                ->editColumn('date', function ($item) {
                    if ($item->date) {
                        return [
                            'display' => $item->date->format('Y-m-d'),
