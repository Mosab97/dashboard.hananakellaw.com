...
use App\Http\Requests\CP\ServiceRequest;
use App\Models\Service;
use App\Services\Filters\ServiceFilterService;
...
class ServiceController extends Controller
{
    ...
    public function __construct(
        Service $_model,
        ServiceFilterService $filterService,
    ) {
        $this->config = config('modules.services');
        $this->_model = $_model;
        ...
        if ($request->isMethod('POST')) {
            ...
            return DataTables::eloquent($items)
                ...
                ->editColumn('icon', function ($item) {
                    return '<img src="' . $item->icon_path . '" alt="' . $item->title . '" class="img-fluid" style="max-width: 100px; max-height: 100px;">';
                })
    ...
    public function edit(Request $request, Service $_model)
    {
    ...
    public function addedit(ServiceRequest $request)
    {
        ...
        try {
            ...
            // dd($validatedData);
            if ($imageFile) {
                $imagePath = Storage::disk('public')->putFile('services', $imageFile);
    ...
    public function delete(Request $request, Service $_model)
    {
        try {
            DB::beginTransaction();
