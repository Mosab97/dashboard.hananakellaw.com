    ...
    class CustomRepeater {
        ...
        addTotalWeightValidation() {
                ...
                validators: {
                    callback: {
                        ...
                        callback: (input) => {
                            ...
                            // Update error message to include current total
                            if (totalWeight !== 100) {
                                this.validator.updateMessage('total_weight_validator', 'callback',
                                    `Total weight must equal 100. Current total: ${totalWeight}`);
                                return false; // Explicitly return false for invalid total
                            }

                            // Update visual indicator
                            this.updateTotalWeightDisplay(totalWeight);
                            return true; // Only return true if exactly 100
                        }
                    }
                }
            });

            // Create or update total weight display with enhanced visibility
            this.createTotalWeightDisplay();

            // Add immediate validation on weight input changes
            this.itemsContainer.addEventListener('input', (e) => {
                if (e.target.matches('input[name$="[weight]"]')) {
                    ...
                    // Update submit button state based on total weight
                    const submitButton = this.container.closest('form').querySelector('[type="submit"]');
                    if (submitButton) {
                        const weightInputs = this.itemsContainer.querySelectorAll(
                            'input[name$="[weight]"]');
                        const totalWeight = Array.from(weightInputs)
                        ...
                        // submitButton.disabled = totalWeight !== 100;
                    }
                }
            });
        }
        ...
        cleanup() {
            if (this.validator) {
                ...
                const titleInputs = this.itemsContainer.querySelectorAll('input[name*="title"]');
                titleInputs.forEach(input => {
                    this.removeValidationField(input.name);
                });

                // Clean up weight fields
                const weightInputs = this.itemsContainer.querySelectorAll('input[name$="[weight]"]');
